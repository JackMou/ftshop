 ...
 * FileId: 11jTIfPXoBKOO/nory4VKYC+9A8215FS
 */
...
public class OrderController extends BaseController {
    ...
    public ResponseEntity<?> review(@ModelAttribute(binding = false) Order order, Boolean passed, @CurrentUser Business currentUser) {
        ...
        if (passed) {
            ...
            try {
                ...
                System.out.println("=====支付订单号========" + orderSn);
                //判断此订单号是否已经发到生产端
                List<OrderItem> orderItems = order.getOrderItems();
                System.out.println("=====支付成功后查询订单商品数量========" + orderItems.size());
                List<Map<String, String>> mapList = new ArrayList<>();
                for (OrderItem orderItem : orderItems) {
                    String proSn = orderItem.getSn().split("_")[0];
                    System.out.println("=====支付成功后商品编号========" + proSn);
                    //查询商品类型和父类型
                    ProductCategory productCategory = productCategoryService.findProductCategoryByProSn(proSn);
                    String treePath = productCategory.getTreePath() + productCategory.getId();
                    System.out.println("=====支付成功后商品类别treePath========" + treePath);
                    //将结算数据放入消息队列
                    Map<String, String> map = new HashMap();
                    map.put("orderId", order.getSn());
                    map.put("orderMoney", orderItem.getPrice().multiply(new BigDecimal(orderItem.getQuantity())).toString());
                    map.put("treePath", treePath);
                    map.put("userId", String.valueOf(order.getMember().getId()));
                    map.put("proSn", proSn);
                    map.put("price", orderItem.getPrice().toString());
                    map.put("quantity", orderItem.getQuantity().toString());

                    System.out.println("=====生产端数据========" + map.toString());
                    mapList.add(map);
                }
                asyncSendNotification.sendOrderSettlement(mapList);
                System.out.println("=====生产数据成功========" + mapList.toString());
            } catch (Exception e) {
                System.out.println("=====生产数据异常========" + e.getMessage());
                e.printStackTrace();
            }
        }
        return Results.OK;
    ...
    public ResponseEntity<?> complete(@ModelAttribute(binding = false) Order order, @CurrentUser Business currentUser) {
        ...
        map.put("status", 5);
        try {
			asyncSendNotification.sendOrderChangeStatus(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
        return Results.OK;
     ...
     */
    @GetMapping("/list")
    public String list(Date beginDate, Date endDate, Order.Type type, Order.Status status, String memberUsername, Boolean isPendingReceive, Boolean isPendingRefunds, Boolean isAllocatedStock, Boolean hasExpired, Pageable pageable, @CurrentStore Store currentStore, ModelMap model) {
        ...
        if (StringUtils.isNotEmpty(memberUsername) && member == null) {
            ...
            model.addAttribute("page", orderService.findPage(beginDate, endDate,type, status, currentStore, member, null, isPendingReceive, isPendingRefunds, null, null, isAllocatedStock, hasExpired, pageable));
        }
     ...
     */
    @GetMapping("/download")
    public ModelAndView listDownload(Date beginDate, Date endDate, Order.Type type, Order.Status status, String memberUsername, Boolean isPendingReceive, Boolean isPendingRefunds, Boolean isAllocatedStock, Boolean hasExpired, Pageable pageable, @CurrentStore Store currentStore, ModelMap model) {
        if(pageable==null){
        ...
        if (StringUtils.isNotEmpty(memberUsername) && member == null) {
            ...
            Page page = orderService.findPage(beginDate, endDate,type, status, currentStore, member, null, isPendingReceive, isPendingRefunds, null, null, isAllocatedStock, hasExpired, pageable);
            List orders = page.getContent();
            for(int i=page.getPageNumber()+1;i<=page.getTotalPages();i++){
                pageable.setPageNumber(i);
                page = orderService.findPage(beginDate, endDate,type, status, currentStore, member, null, isPendingReceive, isPendingRefunds, null, null, isAllocatedStock, hasExpired, pageable);
        ...
        return new ModelAndView(new ExcelView("/business/order/download.xls", filename), model);
    }
