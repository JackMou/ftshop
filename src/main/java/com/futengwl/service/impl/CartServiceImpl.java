 ...
 * FileId: a/q5W//dUq3mCmmC6KZbeg/fEN6uUxkt
 */
...
import com.futengwl.event.CartAddedEvent;
import com.futengwl.event.CartClearedEvent;
import com.futengwl.event.CartMergedEvent;
import com.futengwl.event.CartModifiedEvent;
import com.futengwl.event.CartRemovedEvent;
...
public class CartServiceImpl extends BaseServiceImpl<Cart, Long> implements CartService {
	...
	@Override
	public void add(Cart cart, Sku sku, int quantity,Long recommenderId) {
		...
		addInternal(cart, sku, quantity, recommenderId);
	...
	public void merge(Cart cart) {
		...
		if (anonymousCart != null) {
			for (CartItem cartItem : anonymousCart) {
				...
				Long recommenderId = cartItem.getRecommenderId();
				addInternal(cart, sku, quantity, recommenderId);
			}
	 ...
	 *            数量
	 */
	private void addInternal(Cart cart, Sku sku, int quantity,Long recommenderId) {
		Assert.notNull(cart, "[Assertion failed] - cart is required; it must not be null");
		Assert.isTrue(!cart.isNew(), "[Assertion failed] - cart must be new");
		Assert.notNull(sku, "[Assertion failed] - sku is required; it must not be null");
		Assert.isTrue(!sku.isNew(), "[Assertion failed] - sku must be new");
		...
		if (cart.contains(sku)) {
			...
			cartItem.setRecommenderId(recommenderId);
