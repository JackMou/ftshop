 ...
 * FileId: Sn1ICCBw8hLrbH7M40Ff7FWwQDEswQ4V
 */
...
public abstract class LoginPlugin implements Comparable<LoginPlugin> {
	...
	public void postSignInHandle(LoginPlugin loginPlugin, SocialUser socialUser, String extra, boolean isSignInSuccess, HttpServletRequest request, HttpServletResponse response, ModelAndView modelAndView) throws Exception {
		...
		if(StringUtils.isNotEmpty(redirectUrl)){
            String[] strs=redirectUrl.split("\\?");
            if(null!=strs && strs.length==2){
                redirectUrl="/member/index"+"?"+strs[1];
            }else{
	...
	public String getPreSignInUrl(LoginPlugin loginPlugin) {
		return getPreSignInUrl(loginPlugin, null,null);
	}
	 ...
	 * @return 登录前处理URL
	 */
	public String getPreSignInUrl(LoginPlugin loginPlugin, String extra, String redirectUrl) {
		...
		return setting.getSiteUrl() + "/social_user_login/pre_sign_in_" + loginPlugin.getId() + (StringUtils.isNotEmpty(extra) ? "_" + extra : StringUtils.EMPTY) + (StringUtils.isNotEmpty(redirectUrl)?"?redirectUrl="+redirectUrl:StringUtils.EMPTY);
	}
