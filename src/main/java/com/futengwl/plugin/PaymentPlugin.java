 ...
 * FileId: 4Ry76szn6t52RppV75Q1NWj3zxNDtWim
 */
...
public abstract class PaymentPlugin implements Comparable<PaymentPlugin> {
	...
	public void prePayHandle(PaymentPlugin paymentPlugin, PaymentTransaction paymentTransaction, String paymentDescription, String extra, HttpServletRequest request, HttpServletResponse response, ModelAndView modelAndView) throws Exception {
		modelAndView.setViewName("redirect:" + paymentPlugin.getPayUrl(paymentPlugin, paymentTransaction, extra));
	}
	...
	public String getPrePayUrl(PaymentPlugin paymentPlugin, PaymentTransaction paymentTransaction, String extra) {
		...
		Assert.hasText(paymentPlugin.getId(), "[Assertion failed] - paymentPlugin id must have text; it must not be null, empty, or blank");
		Assert.notNull(paymentTransaction, "[Assertion failed] - paymentTransaction is required; it must not be null");
		Assert.hasText(paymentTransaction.getSn(), "[Assertion failed] - paymentTransaction sn must have text; it must not be null, empty, or blank");

		Setting setting = SystemUtils.getSetting();
		return setting.getSiteUrl() + "/payment/pre_pay_" + paymentTransaction.getSn() + (StringUtils.isNotEmpty(extra) ? "_" + extra : StringUtils.EMPTY);
	}
	...
	public String getPayUrl(PaymentPlugin paymentPlugin, PaymentTransaction paymentTransaction, String extra) {
		...
		Setting setting = SystemUtils.getSetting();
		return setting.getSiteUrl() + "/payment/pay_" + paymentTransaction.getSn() + (StringUtils.isNotEmpty(extra) ? "_" + extra : StringUtils.EMPTY);
	}
	...
	public String getPostPayUrl(PaymentPlugin paymentPlugin, PaymentTransaction paymentTransaction, String extra) {
		...
		Setting setting = SystemUtils.getSetting();
		return setting.getSiteUrl() + "/payment/post_pay_" + paymentTransaction.getSn() + (StringUtils.isNotEmpty(extra) ? "_" + extra : StringUtils.EMPTY);
	}
	...
	protected String joinValue(Map<String, Object> map, String prefix, String suffix, String separator, boolean ignoreEmptyValue, String... ignoreKeys) {
		...
		if (map != null) {
			for (Map.Entry<String, Object> entry : map.entrySet()) {
				...
				String value = ConvertUtils.convert(entry.getValue());
				if (StringUtils.isNotEmpty(key) && !ArrayUtils.contains(ignoreKeys, key) && (!ignoreEmptyValue || StringUtils.isNotEmpty(value))) {
					list.add(value != null ? value : StringUtils.EMPTY);
				}
			}
		}
		return (prefix != null ? prefix : StringUtils.EMPTY) + StringUtils.join(list, separator) + (suffix != null ? suffix : StringUtils.EMPTY);
