...
<html>
	<head>
		...
		<title>${message("shop.cart.title")}[#if showPowered] - Powered By FTSHOP[/#if]</title>
		...
		<link href="${base}/resources/mobile/shop/css/cart/iconfont/iconfont.css" rel="stylesheet">
		...
		<script src="${base}/resources/mobile/shop/js/cart/carts.js"></script>
		<script src="${base}/resources/mobile/shop/js/cart/fontsize.js"></script>
		<script src="${base}/resources/mobile/shop/js/cart/public.js"></script>
		...
		[#noautoesc]
			[#escape x as x?js_string]
				...
				$().ready(function() {
					...
					// 修改
					$document.on("success.ftshop.modifyCart", function(event, data) {
					...
					// 移除
					$document.on("success.ftshop.removeCart", function(event, data) {
						var $element = $(event.target);
					...
					// 清空
					$document.on("success.ftshop.clearCart", function() {
						location.reload(true);
					});
	...
	$(function(){
		fnSelect(".select_all", ".media-list", ".check");
	});
	</script>
	...
	<body class="shop cart-list">
		...
		<main>
			<div class="container-fluid">
							...
							<div class="panel-body">
								<ul class="media-list">
									[#list cartItems as cartItem]
										<li class="media">
											<div class="carts" style=" float: left; width: 10%; margin-top: 6%;">
												<div class="carts_check"><i class="iconfont check[#if cartItem.isChecked] checked[/#if]" id="${cartItem.id}"></i></div>
											</div>
											...
											<div class="media-right media-middle text-right">
												<button class="btn btn-default btn-sm" type="button" data-action="removeCart" data-sku-id="${cartItem.sku.id}">${message("common.delete")}</button>
											</div>
										</li>
									[/#list]
								</ul>
				...
				[#else]
					<p class="empty">
						<a href="${base}/store/${storeId}">${message("shop.cart.empty")}</a>
					</p>
				[/#if]
			</div>
		</main>
		<footer class="footer-default" data-spy="scrollToFixed" data-bottom="0">
			<div class="container-fluid">
				<div class="row">
					<div class="col-xs-8 text-left">
						...
						<div class="foot bg-white select_all">
							<div class="p10 ml10 mr10 clearfix f26">
								<div class="fl"><i class="iconfont check vertical_middle"></i>
									<span for='checkAll' class="vertical_middle ml10">全选</span>
									<span>
					...
					<div class="col-xs-4">
						<button id="clear" class="clear btn btn-orange btn-lg btn-block" type="button" data-action="clearCart">${message("common.delete")}</button>
					</div>
					<div class="col-xs-4 fr">
						<button id="checkOut" class="btn btn-red btn-lg btn-block" type="button" data-action="checkout"[#if !currentCart?? || currentCart.isEmpty()] disabled[/#if]>${message("shop.cart.checkout")}</button>
					</div>
				</div>
			</div>
		</footer>
