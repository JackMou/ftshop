...
<head>
	...
	<title>${message("shop.order.checkout")}[#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			<script>
			$().ready(function() {
				[#if removeSpecailProduct?has_content]
				    [#if removeSpecailProduct?has_content]
                    [#if removeSpecailProduct=="1"]
			    		alert("298专享礼包只能购买一次，已经从订单中移除。");
			    		history.go(-1);
			    	[/#if]
                    [#if removeSpecailProduct=="2"]
						alert("您已经购买过298专享礼包，本商品每个顾客只能购买一次。");
						history.go(-1);
					[/#if]
					[#if removeSpecailProduct=="3"]
						alert("该活动商品每个用户仅能购买一次!");
						history.go(-1);
                    [/#if]
                [/#if]
				[/#if]
				...
				var amount = ${amount!0};
				var amountPayable = ${amountPayable!0};
                        ...
                        window.wx.ready(function() {
                            ...
                            window.wx.getLocation({
                                ...
                                success: function (res) {
                                    ...
                                    if(latitude){
                                        $.ajax({
                                            url: '${base}/common/area/location',
                                            methods: 'get',
                                            data: {
                                                location: latitude+','+longitude
                                            },
                                            success: function(res) {
                                                $areaId.attr("value",res.areaId);
                                                $areaId.attr("treePath",res.areaTreePath);
                                                // 地区选择
                                                $areaId.lSelect({
				...
				// 添加收货地址成功
				$addReceiverForm.on("success.ftshop.ajaxSubmit", function(event, data) {
