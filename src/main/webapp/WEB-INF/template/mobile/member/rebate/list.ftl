...
<head>
    ...
    <title>${message("business.order.list")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
			...
			<script>
                $().ready(function () {
                    ...
                    $export.click(function () {
                        ...
                        var form = $('<form method="get" action="${base}/business/order/download"></form>');
                        form.appendTo('body');
                        if (typeof($("#orderProperty").val()) != "undefined") {
                            var input = $('<input name="orderProperty" type="hidden" value="' + $("#orderProperty").val() + '">');
                            form.append(input);
                        }
                        if (typeof($("#orderDirection").val()) != "undefined") {
                            input = $('<input name="orderDirection" type="hidden" value="' + $("#orderDirection").val() + '">');
                            form.append(input);
                        }
                        if (typeof($("#isPendingReceive").val()) != "undefined") {
                            input = $('<input name="isPendingReceive" type="hidden" value="' + $("#isPendingReceive").val() + '">');
                            form.append(input);
                        }
                        if (typeof($("#isPendingReceive").val()) != "undefined") {
                            input = $('<input name="isPendingReceive" type="hidden" value="' + $("#isPendingReceive").val() + '">');
                        ...
                        if (typeof($("#isPendingRefunds").val()) != "undefined") {
                            input = $('<input name="isPendingRefunds" type="hidden" value="' + $("#isPendingRefunds").val() + '">');
                            form.append(input);
                        }
                        if (typeof($("#isAllocatedStock").val()) != "undefined") {
                            input = $('<input name="isAllocatedStock" type="hidden" value="' + $("#isAllocatedStock").val() + '">');
                            form.append(input);
                        }
                        if (typeof($("#hasExpired").val()) != "undefined") {
                            input = $(' <input name="hasExpired" type="hidden" value="' + $("#hasExpired").val() + '">');
                            form.append(input);
                        }
                        ...
                        if (typeof($("#type").val()) != "undefined") {
                            input = $(' <input name="type" type="hidden" value="' + $("#type").val() + '">');
                            form.append(input);
                        }
                        if (typeof($("#status").val()) != "undefined") {
                            input = $(' <input name="status" type="hidden" value="' + $("#status").val() + '">');
                            form.append(input);
                        }
                    ...
                    [#--$.ajax({--]
                    [#--type: "GET",--]
                    [#--url: "${base}/business/order/download",--]
                    [#--data:--]
                    [#--{--]
                    [#--searchProperty: $("#searchProperty").val(),--]
                    [#--orderProperty: $("#orderProperty").val(),--]
                    [#--orderDirection: $("#orderDirection").val(),--]
                    [#--isPendingReceive: $("#isPendingReceive").val(),--]
                    [#--isPendingRefunds: $("#isPendingRefunds").val(),--]
                    [#--isAllocatedStock: $("#isAllocatedStock").val(),--]
                    [#--hasExpired: $("#hasExpired").val(),--]
                    [#--status: $("#status").val(),--]
                    [#--type: $("#type").val(),--]
...
<main>
    <div class="container-fluid">
        ...
        <form action="${base}/business/order/list" method="get">
            ...
            <input name="isPendingReceive" type="hidden"
                   value="[#if isPendingReceive??]${isPendingReceive?string("true", "false")}[/#if]">
            <input name="isPendingRefunds" type="hidden"
                   value="[#if isPendingRefunds??]${isPendingRefunds?string("true", "false")}[/#if]">
            <input name="isAllocatedStock" type="hidden"
                   value="[#if isAllocatedStock??]${isAllocatedStock?string("true", "false")}[/#if]">
            <input name="hasExpired" type="hidden"
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body form-horizontal">
                                ...
                                <div class="col-xs-9 col-sm-7">
                                    <select id="type" name="type" class="selectpicker form-control" data-size="5">
                                        <option value="">${message("common.choose")}</option>
											[#list types as value]
												<option value="${value}"[#if value == type]
                                                        selected[/#if]>${message("Order.Type." + value)}</option>
                                            [/#list]
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-xs-9 col-sm-7">
                                    <select id="status" name="status" class="selectpicker form-control" data-size="5">
                                        <option value="">${message("common.choose")}</option>
											[#list statuses as value]
												<option value="${value}"[#if value == status]
                                                        selected[/#if]>${message("Order.Status." + value)}</option>
                                            [/#list]
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-xs-9 col-sm-7">
                                    <div class="input-group" data-provide="datetimerangepicker"
                                        ...
                                        <input id="beginDate" name="beginDate" class="form-control"
                                               type="text" [#if beginDate??]
                                               value="${beginDate?string("yyyy-MM-dd HH:mm:ss")}"[/#if]>
                                        <span class="input-group-addon">-</span>
                                        <input id="endDate" name="endDate" class="form-control"
                                               type="text" [#if endDate??]
                                               value="${endDate?string("yyyy-MM-dd HH:mm:ss")}"[/#if]>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="btn-group">
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li[#if isPendingReceive?? && isPendingReceive] class="active"[/#if]
                                                                                        data-filter-property="isPendingReceive"
                                        ...
                                        <li class="divider"></li>
                                        <li[#if isPendingRefunds?? && isPendingRefunds] class="active"[/#if]
                                                                                        data-filter-property="isPendingRefunds"
                                                                                        data-filter-value="true">
                                        ...
                                        <li class="divider"></li>
                                        <li[#if isAllocatedStock?? && isAllocatedStock] class="active"[/#if]
                                                                                        data-filter-property="isAllocatedStock"
                                                                                        data-filter-value="true">
                                        ...
                                        <li class="divider"></li>
                                        <li[#if hasExpired?? && hasExpired] class="active"[/#if]
                                                                            data-filter-property="hasExpired"
                                                                            data-filter-value="true">
                        ...
                        <div class="col-xs-12 col-sm-3">
                            <div id="search" class="input-group">
                                <div class="input-group-btn">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li[#if !page.searchProperty?? || page.searchProperty == "sn"]
                                                class="active"[/#if] data-search-property="sn">
                                            <a href="javascript:;">${message("Order.sn")}</a>
                                        </li>
                                        <li[#if page.searchProperty == "consignee"] class="active"[/#if]
                                                                                    data-search-property="consignee">
                                            <a href="javascript:;">${message("Order.consignee")}</a>
                                        </li>
                                        <li[#if page.searchProperty == "areaName"] class="active"[/#if]
                                                                                   data-search-property="areaName">
                                            <a href="javascript:;">${message("Order.area")}</a>
                                        </li>
                                        <li[#if page.searchProperty == "address"] class="active"[/#if]
                                                                                  data-search-property="address">
                                            <a href="javascript:;">${message("Order.address")}</a>
                                        </li>
                                        <li[#if page.searchProperty == "zipCode"] class="active"[/#if]
                                                                                  data-search-property="zipCode">
                                            <a href="javascript:;">${message("Order.zipCode")}</a>
                                        </li>
                                        <li[#if page.searchProperty == "phone"] class="active"[/#if]
                                                                                data-search-property="phone">
								...
								[#if page.content?has_content]
									<tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span title="${order.createdDate?string("yyyy-MM-dd HH:mm:ss")}"
                                                      data-toggle="tooltip">${order.createdDate}</span>
                                            </td>
                                            <td>
                                                <a class="btn btn-default btn-xs btn-icon"
                                                   href="${base}/business/order/view?orderId=${order.id}"
                                                   title="${message("common.view")}" data-toggle="tooltip">
                                                    <i class="iconfont icon-search"></i>
                                                </a>
													[#if !order.hasExpired() && (order.status == "PENDING_PAYMENT" || order.status == "PENDING_REVIEW")]
														<a class="btn btn-default btn-xs btn-icon"
                                                           href="${base}/business/order/edit?orderId=${order.id}"
                                                           title="${message("common.edit")}" data-toggle="tooltip"
                                                           data-redirect-url>
                                                            <i class="iconfont icon-write"></i>
                                                        </a>
                                                    [#else]
														<button class="btn btn-default btn-xs btn-icon" type="button"
                                                                title="${message("business.order.editNotAllowed")}"
                                                                data-toggle="tooltip" disabled>
                                                            <i class="iconfont icon-write"></i>
                                                        </button>
                                                    [/#if]
                                                <a class="print btn btn-default btn-xs btn-icon" href="javascript:;"
                                                   title="${message("business.order.print")}" data-toggle="tooltip"
                                                   data-order-id="${order.id}"
                                                   data-is-delivery="${order.isDelivery?string("true", "false")}">
