...
<head>
	...
	<title>${message("member.login.title")}[#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			<script>
				...
				// 用户登录成功、记住用户名
				$loginForm.on("success.ftshop.ajaxSubmit", function() {
				...
				// 验证码图片
				$loginForm.on("error.ftshop.ajaxSubmit", function() {
                ...
                $sendCode.on('click', function() {
                    ...
                    $.get('${base}/member/register/authCode?username=' + $username.val(), function(res) {
                        if(res === 'success') {
                            sending = true;
			...
			<form id="loginForm" action="${base}/member/login" method="post">
				...
				<div class="form-group">
					<div class="input-group">
						...
						<input id="password" name="password" class="form-control" type="text" maxlength="10" placeholder="${message("member.login.authcodePlaceholder")}" autocomplete="off">
                        <span class="input-group-btn">
                            <button id="sendCode" class="btn btn-default" type="button">${message("member.register.sendAuthCode")}</button>
                        </span>
					</div>
				</div>
	...
	<footer class="text-center">
		[#if loginPlugins?has_content && !socialUserId?has_content && !uniqueId?has_content]
			[#list loginPlugins as loginPlugin]
				<a href="${base}/social_user_login?loginPluginId=${loginPlugin.id}"[#if loginPlugin.description??] id="weixin" title="${loginPlugin.description}"[/#if]>
