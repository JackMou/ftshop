...
<head>
	...
	<title>${message("admin.attribute.edit")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.attribute.edit")}</li>
			</ol>
			<form id="attributeForm" class="ajax-form form-horizontal" action="${base}/admin/attribute/update" method="post">
				<input name="id" type="hidden" value="${attribute.id}">
				<div class="panel panel-default">
					<div class="panel-heading">${message("admin.attribute.edit")}</div>
					<div class="panel-body">
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="name">${message("Attribute.name")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="name" name="name" class="form-control" type="text" value="${attribute.name}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label">${message("Attribute.productCategory")}:</label>
							<div class="col-xs-9 col-sm-4">
								<p class="form-control-static">${attribute.productCategory.name}</p>
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label" for="order">${message("common.order")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="order" name="order" class="form-control" type="text" value="${attribute.order}" maxlength="9">
							</div>
						</div>
						<div class="form-group">
							<div class="col-xs-9 col-sm-4 col-xs-offset-3 col-sm-offset-2">
