...
<head>
	...
	<title>${message("admin.auditLog.list")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $clearToggle = $("[data-toggle='clear']");
				
				$clearToggle.click(function() {
					bootbox.confirm("${message("common.clearConfirm")}", function(result) {
						if (result == null || !result) {
							return;
						}
						
						$.ajax({
							url: "${base}/admin/audit_log/clear",
							type: "POST",
							dataType: "json",
							cache: false,
							success: function() {
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.auditLog.list")}</li>
			</ol>
			<form action="${base}/admin/audit_log/list" method="get">
						...
						<div class="row">
							<div class="col-xs-12 col-sm-9">
								<div class="btn-group">
									...
									<button class="btn btn-default" type="button" data-toggle="clear">
									...
									<div class="input-group-btn">
										<button class="btn btn-default" type="button" data-toggle="dropdown">
											[#switch page.searchProperty]
												[#default]
													<span>${message("AuditLog.detail")}</span>
											[/#switch]
											<span class="caret"></span>
										</button>
										<ul class="dropdown-menu">
											<li[#if !page.searchProperty?? || page.searchProperty == "detail"] class="active"[/#if] data-search-property="detail">
												<a href="javascript:;">${message("AuditLog.detail")}</a>
											</li>
										</ul>
									</div>
							...
							<table class="table table-hover">
								<thead>
									<tr>
										...
										<th>
											<a href="javascript:;" data-order-property="action">
												${message("AuditLog.action")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
											<a href="javascript:;" data-order-property="detail">
												${message("AuditLog.detail")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
											<a href="javascript:;" data-order-property="ip">
												${message("AuditLog.ip")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
											<a href="javascript:;" data-order-property="user">
									...
									<tbody>
										[#list page.content as auditLog]
											<tr>
												<td>
													<div class="checkbox">
														<input name="ids" type="checkbox" value="${auditLog.id}">
														<label></label>
													</div>
												</td>
												...
												<td>
													[#if auditLog.detail??]
														<span title="${auditLog.detail}">${abbreviate(auditLog.detail, 30, "...")}</span>
													[/#if]
												</td>
												<td>${auditLog.ip}</td>
												<td>[#if auditLog.user??]${auditLog.user.displayName}[/#if]</td>
												<td>
													<span title="${auditLog.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${auditLog.createdDate}</span>
												</td>
												<td>
													<a class="btn btn-default btn-xs btn-icon" href="${base}/admin/audit_log/view?id=${auditLog.id}" title="${message("common.view")}" data-toggle="tooltip">
														<i class="iconfont icon-search"></i>
													</a>
												</td>
											</tr>
