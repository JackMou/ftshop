...
<head>
	...
	<title>${message("admin.store.add")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
		...
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				$businessId.selectpicker({
					...
					ajax: {
						url: "${base}/admin/store/business_select",
						type: "GET",
						data: function() {
							return {
								keyword: "{{{q}}}"
					...
					<div class="panel-body">
						<div class="form-group">
							...
							<div class="col-xs-9 col-sm-4">
								<select id="businessId" name="businessId" class="form-control" title="${message("admin.store.businessTitle")}"></select>
							</div>
						</div>
						<div class="form-group">
