...
<head>
	...
	<title>${message("admin.plugin.pay99billPayment.setting")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $pay99billPaymentForm = $("#pay99billPaymentForm");
				
				// 表单验证
				$pay99billPaymentForm.validate({
					rules: {
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.plugin.pay99billPayment.setting")}</li>
			</ol>
			<form id="pay99billPaymentForm" class="ajax-form form-horizontal" action="${base}/admin/plugin/pay_99bill_payment/update" method="post">
				<div class="panel panel-default">
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="partner">${message("admin.plugin.pay99billPayment.partner")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="partner" name="partner" class="form-control" type="text" value="${pluginConfig.getAttribute("partner")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="key">${message("admin.plugin.pay99billPayment.key")}:</label>
							<div class="col-xs-9 col-sm-4">
