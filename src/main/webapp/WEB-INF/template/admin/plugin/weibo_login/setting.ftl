...
<head>
	...
	<title>${message("admin.plugin.weiboLogin.setting")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $weiboLoginForm = $("#weiboLoginForm");
				
				// 表单验证
				$weiboLoginForm.validate({
					rules: {
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.plugin.weiboLogin.setting")}</li>
			</ol>
			<form id="weiboLoginForm" class="ajax-form form-horizontal" action="${base}/admin/plugin/weibo_login/update" method="post">
				<div class="panel panel-default">
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="oauthKey">${message("admin.plugin.weiboLogin.oauthKey")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="oauthKey" name="oauthKey" class="form-control" type="text" value="${pluginConfig.getAttribute("oauthKey")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="oauthSecret">${message("admin.plugin.weiboLogin.oauthSecret")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="oauthSecret" name="oauthSecret" class="form-control" type="text" value="${pluginConfig.getAttribute("oauthSecret")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
