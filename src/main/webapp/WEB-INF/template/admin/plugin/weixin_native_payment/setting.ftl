...
<head>
	...
	<title>${message("admin.plugin.weixinNativePayment.setting")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $weixinNativePaymentForm = $("#weixinNativePaymentForm");
				
				// 表单验证
				$weixinNativePaymentForm.validate({
					rules: {
			...
			<form id="weixinNativePaymentForm" class="ajax-form form-horizontal" action="${base}/admin/plugin/weixin_native_payment/update" method="post">
				<div class="panel panel-default">
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="appId">${message("admin.plugin.weixinNativePayment.appId")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="appId" name="appId" class="form-control" type="text" value="${pluginConfig.getAttribute("appId")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="mchId">${message("admin.plugin.weixinNativePayment.mchId")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="mchId" name="mchId" class="form-control" type="text" value="${pluginConfig.getAttribute("mchId")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="apiKey">${message("admin.plugin.weixinNativePayment.apiKey")}:</label>
							<div class="col-xs-9 col-sm-4">
