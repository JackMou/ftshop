...
<head>
	...
	<title>${message("admin.plugin.pay99billBankPayment.setting")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $pay99billBankPaymentForm = $("#pay99billBankPaymentForm");
				
				// 表单验证
				$pay99billBankPaymentForm.validate({
					rules: {
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.plugin.pay99billBankPayment.setting")}</li>
			</ol>
			<form id="pay99billBankPaymentForm" class="ajax-form form-horizontal" action="${base}/admin/plugin/pay_99bill_bank_payment/update" method="post">
				<div class="panel panel-default">
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="partner">${message("admin.plugin.pay99billBankPayment.partner")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="partner" name="partner" class="form-control" type="text" value="${pluginConfig.getAttribute("partner")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="key">${message("admin.plugin.pay99billBankPayment.key")}:</label>
							<div class="col-xs-9 col-sm-4">
