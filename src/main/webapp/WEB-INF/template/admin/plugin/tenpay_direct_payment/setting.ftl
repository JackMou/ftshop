...
<head>
	...
	<title>${message("admin.plugin.tenpayDirectPayment.setting")} - Powered By FTSHOP</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $tenpayDirectPaymentForm = $("#tenpayDirectPaymentForm");
				
				// 表单验证
				$tenpayDirectPaymentForm.validate({
					rules: {
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.plugin.tenpayDirectPayment.setting")}</li>
			</ol>
			<form id="tenpayDirectPaymentForm" class="ajax-form form-horizontal" action="${base}/admin/plugin/tenpay_direct_payment/update" method="post">
				<div class="panel panel-default">
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="partner">${message("admin.plugin.tenpayDirectPayment.partner")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="partner" name="partner" class="form-control" type="text" value="${pluginConfig.getAttribute("partner")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="key">${message("admin.plugin.tenpayDirectPayment.key")}:</label>
							<div class="col-xs-9 col-sm-4">
