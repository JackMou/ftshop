...
<head>
	...
	<title>${message("admin.plugin.qqLogin.setting")}[#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $qqLoginForm = $("#qqLoginForm");
				
				// 表单验证
				$qqLoginForm.validate({
					rules: {
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.plugin.qqLogin.setting")}</li>
			</ol>
			<form id="qqLoginForm" class="ajax-form form-horizontal" action="${base}/admin/plugin/qq_login/update" method="post">
				<div class="panel panel-default">
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="oauthKey">${message("admin.plugin.qqLogin.oauthKey")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="oauthKey" name="oauthKey" class="form-control" type="text" value="${pluginConfig.getAttribute("oauthKey")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="oauthSecret">${message("admin.plugin.qqLogin.oauthSecret")}:</label>
							<div class="col-xs-9 col-sm-4">
