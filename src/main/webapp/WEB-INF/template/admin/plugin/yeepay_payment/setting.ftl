...
<head>
	...
	<title>${message("admin.plugin.yeepayPayment.setting")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $yeepayPaymentForm = $("#yeepayPaymentForm");
				
				// 表单验证
				$yeepayPaymentForm.validate({
					rules: {
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.plugin.yeepayPayment.setting")}</li>
			</ol>
			<form id="yeepayPaymentForm" class="ajax-form form-horizontal" action="${base}/admin/plugin/yeepay_payment/update" method="post">
				<div class="panel panel-default">
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="partner">${message("admin.plugin.yeepayPayment.partner")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="partner" name="partner" class="form-control" type="text" value="${pluginConfig.getAttribute("partner")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="key">${message("admin.plugin.yeepayPayment.key")}:</label>
							<div class="col-xs-9 col-sm-4">
