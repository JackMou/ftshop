...
<head>
	...
	<title>${message("admin.plugin.paypalPayment.setting")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $paypalPaymentForm = $("#paypalPaymentForm");
			...
			<form id="paypalPaymentForm" class="ajax-form form-horizontal" action="${base}/admin/plugin/paypal_payment/update" method="post">
				<div class="panel panel-default">
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label item-required" for="partner">${message("admin.plugin.paypalPayment.partner")}:</label>
							<div class="col-xs-9 col-sm-4">
								<input id="partner" name="partner" class="form-control" type="text" value="${pluginConfig.getAttribute("partner")}" maxlength="200">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-3 col-sm-2 control-label">${message("admin.plugin.paypalPayment.currency")}:</label>
							<div class="col-xs-9 col-sm-4">
								<select name="currency" class="selectpicker form-control" data-size="10">
									[#list currencies as currency]
										<option value="${currency}"[#if currency == pluginConfig.getAttribute("currency")] selected[/#if]>${message("PaypalPlugin.Currency." + currency)}</option>
									[/#list]
								</select>
							</div>
						</div>
