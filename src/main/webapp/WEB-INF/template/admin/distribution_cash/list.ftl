...
<head>
	...
	<title>${message("admin.distributionCash.list")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
			...
			$().ready(function() {
				...
				$review.click(function() {
					...
					bootbox.prompt({
						...
						inputOptions: [
							{
								text: "${message("admin.distributionCash.reviewApproved")}",
								value: "APPROVED"
							},
							{
								text: "${message("admin.distributionCash.reviewFailed")}",
								value: "FAILED"
							}
						],
						callback: function(result) {
							...
							$.ajax({
								url: "${base}/admin/distribution_cash/review",
								type: "POST",
								data: {
									id: $element.data("id"),
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.distributionCash.list")}</li>
			</ol>
			<form action="${base}/admin/distribution_cash/list" method="get">
									...
									<div class="input-group-btn">
										<button class="btn btn-default" type="button" data-toggle="dropdown">
											[#switch page.searchProperty]
												[#default]
													<span>${message("DistributionCash.distributor")}</span>
											[/#switch]
											<span class="caret"></span>
										</button>
										<ul class="dropdown-menu">
											<li[#if !page.searchProperty?? || page.searchProperty == "distributor.member.username"] class="active"[/#if] data-search-property="distributor.member.username">
												<a href="javascript:;">${message("DistributionCash.distributor")}</a>
											</li>
										</ul>
									</div>
							...
							<table class="table table-hover">
								<thead>
									<tr>
										...
										<th>
											<a href="javascript:;" data-order-property="accountHolder">
												${message("DistributionCash.accountHolder")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
											<a href="javascript:;" data-order-property="distributor">
												${message("DistributionCash.distributor")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
								...
								[#if page.content?has_content]
									<tbody>
										[#list page.content as distributionCash]
											<tr[#if !distributionCash_has_next] class="last"[/#if]>
												...
												<td>${distributionCash.distributor.member.username}</td>
												<td>
												...
												<td>
													<span title="${distributionCash.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${distributionCash.createdDate}</span>
												</td>
												<td>
													[#if distributionCash.status == "PENDING"]
														<a class="review btn btn-default btn-xs btn-icon" href="javascript:;" title="${message("common.review")}" data-toggle="tooltip" data-id="${distributionCash.id}">
															<i class="iconfont icon-comment"></i>
														</a>
													[#else]
														<button class="btn btn-default btn-xs btn-icon" type="button" title="${message("DistributionCash.Status." + distributionCash.status)}" data-toggle="tooltip" disabled>
															<i class="iconfont icon-comment"></i>
														</button>
													[/#if]
												</td>
