...
<head>
	...
	<title>${message("admin.productCategory.edit")}[#if showPowered] - Powered By FTSHOP[/#if]</title>
							...
							<div class="col-xs-9 col-sm-4">
								<select id="promotionIds" name="promotionIds" class="form-control" title="${message("common.choose")}" multiple>
									[#list productCategory.promotions as promotion]
										<option value="${promotion.id}" data-subtext="[${promotion.store.name}]">${promotion.name}</option>
									[/#list]
								</select>
							</div>
						</div>
