...
<head>
	...
	<title>${message("admin.orderRefunds.list")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.orderRefunds.list")}</li>
			</ol>
			<form action="${base}/admin/order_refunds/list" method="get">
									...
									<div class="input-group-btn">
										<button class="btn btn-default" type="button" data-toggle="dropdown">
											[#switch page.searchProperty]
												[#case "account"]
													<span>${message("OrderRefunds.account")}</span>
													[#break /]
												[#case "payee"]
													<span>${message("OrderRefunds.payee")}</span>
													[#break /]
												[#default]
													<span>${message("OrderRefunds.sn")}</span>
											[/#switch]
											<span class="caret"></span>
										</button>
										<ul class="dropdown-menu">
											<li[#if !page.searchProperty?? || page.searchProperty == "sn"] class="active"[/#if] data-search-property="sn">
												<a href="javascript:;">${message("OrderRefunds.sn")}</a>
											</li>
											<li[#if page.searchProperty == "account"] class="active"[/#if] data-search-property="account">
												<a href="javascript:;">${message("OrderRefunds.account")}</a>
											</li>
											<li[#if page.searchProperty == "payee"] class="active"[/#if] data-search-property="payee">
												<a href="javascript:;">${message("OrderRefunds.payee")}</a>
											</li>
										</ul>
									</div>
							...
							<table class="table table-hover">
								<thead>
									<tr>
										...
										<th>
											<a href="javascript:;" data-order-property="method">
										...
										<th>
											<a href="javascript:;" data-order-property="payee">
									...
									<tbody>
										[#list page.content as refunds]
											<tr>
												<td>
													<div class="checkbox">
														<input name="ids" type="checkbox" value="${refunds.id}">
														<label></label>
													</div>
												</td>
												<td>${refunds.sn}</td>
												<td>${message("OrderRefunds.Method." + refunds.method)}</td>
												<td>${refunds.paymentMethod}</td>
												<td>${currency(refunds.amount, true)}</td>
												<td>${refunds.payee}</td>
												<td>${refunds.order.sn}</td>
												<td>
													<span title="${refunds.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${refunds.createdDate}</span>
												</td>
												<td>
													<a class="btn btn-default btn-xs btn-icon" href="${base}/admin/order_refunds/view?id=${refunds.id}" title="${message("common.view")}" data-toggle="tooltip">
														<i class="iconfont icon-search"></i>
													</a>
												</td>
											</tr>
