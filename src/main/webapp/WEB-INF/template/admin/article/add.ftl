...
<head>
	...
	<title>${message("admin.article.add")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.article.add")}</li>
			</ol>
			<form id="articleForm" class="ajax-form form-horizontal" action="${base}/admin/article/save" method="post">
				<div class="panel panel-default">
					<div class="panel-heading">${message("admin.article.add")}</div>
							...
							<div class="col-xs-9 col-sm-4">
								<select name="articleCategoryId" class="selectpicker form-control" data-size="10">
									[#list articleCategoryTree as articleCategory]
										<option value="${articleCategory.id}" title="${articleCategory.name}">
						...
						<div class="form-group">
							...
							<div class="col-xs-9 col-sm-10">
								[#list articleTags as articleTag]
									<div class="checkbox checkbox-inline">
										<input id="articleTag_${articleTag.id}" name="articleTagIds" type="checkbox" value="${articleTag.id}">
