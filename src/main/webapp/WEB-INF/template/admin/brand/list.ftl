...
<head>
	...
	<title>${message("admin.brand.list")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.brand.list")}</li>
			</ol>
			<form action="${base}/admin/brand/list" method="get">
					...
					<div class="panel-heading">
						<div class="row">
							<div class="col-xs-12 col-sm-9">
								<div class="btn-group">
									<a class="btn btn-default" href="${base}/admin/brand/add" data-redirect-url="${base}/admin/brand/list">
									...
									<div class="input-group-btn">
										...
										<ul class="dropdown-menu">
											<li[#if !page.searchProperty?? || page.searchProperty == "name"] class="active"[/#if] data-search-property="name">
												<a href="javascript:;">${message("Brand.name")}</a>
											</li>
											<li[#if page.searchProperty == "url"] class="active"[/#if] data-search-property="url">
												<a href="javascript:;">${message("Brand.url")}</a>
											</li>
										</ul>
									</div>
									...
									<tbody>
										[#list page.content as brand]
											<tr>
												<td>
													<div class="checkbox">
														<input name="ids" type="checkbox" value="${brand.id}">
														<label></label>
													</div>
												</td>
												<td>${brand.name}</td>
												<td>
													[#if brand.type == "IMAGE"]
														<a href="${brand.logo}" target="_blank">${message("common.view")}</a>
													[#else]
														-
													[/#if]
												</td>
												...
												<td>${brand.order}</td>
												<td>
													<a class="btn btn-default btn-xs btn-icon" href="${base}/admin/brand/edit?id=${brand.id}" title="${message("common.edit")}" data-toggle="tooltip" data-redirect-url>
														<i class="iconfont icon-write"></i>
													</a>
													<a class="btn btn-default btn-xs btn-icon" href="${base}${brand.path}" title="${message("common.view")}" data-toggle="tooltip" target="_blank">
														<i class="iconfont icon-search"></i>
													</a>
												</td>
											</tr>
