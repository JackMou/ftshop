...
<head>
	...
	<title>${message("admin.review.list")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.review.list")}</li>
			</ol>
			<form action="${base}/admin/review/list" method="get">
								...
								<div class="btn-group">
									...
									<div class="btn-group">
										<button class="btn btn-default dropdown-toggle" type="button" data-toggle="dropdown">
											...
											[#else]
												${message("admin.review.allType")}
											[/#if]
											<span class="caret"></span>
										</button>
										<ul class="dropdown-menu">
											<li[#if !type??] class="active"[/#if] data-filter-property="type">
												<a href="javascript:;">${message("admin.review.allType")}</a>
											</li>
											[#list types as item]
									...
									<tbody>
										[#list page.content as review]
											<tr>
												<td>
													<div class="checkbox">
														<input name="ids" type="checkbox" value="${review.id}">
														<label></label>
													</div>
												</td>
												<td>
												...
												<td>${review.score}</td>
												<td>
													<span title="${review.content}">${abbreviate(review.content, 30, "...")}</span>
												</td>
												<td>
													<span title="${review.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${review.createdDate}</span>
												</td>
												<td>
													[#if review.isShow]
												...
												<td>
													<a class="btn btn-default btn-xs btn-icon" href="${base}/admin/review/edit?id=${review.id}" title="${message("common.edit")}" data-toggle="tooltip" data-redirect-url>
														<i class="iconfont icon-write"></i>
													</a>
													<a class="btn btn-default btn-xs btn-icon" href="${base}${review.path}" title="${message("common.view")}" data-toggle="tooltip" target="_blank">
														<i class="iconfont icon-search"></i>
													</a>
												</td>
											</tr>
