...
<head>
	...
	<title>${message("admin.member.list")}[#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$export.click(function () {
                        ...
                        var form = $('<form method="get" action="${base}/admin/member/download"></form>');
                        form.appendTo('body');
                        
                        form.append('<input name="orderProperty" type="hidden" value="${page.orderProperty}">');
                        form.append('<input name="orderDirection" type="hidden" value="${page.orderDirection}">');
                        form.append('<input name="pageSize" type="hidden" value="${page.pageSize}">');
                        form.append('<input name="searchProperty" type="hidden" value="${page.searchProperty}">');
                        form.append('<input name="searchValue" type="hidden" value="${page.searchValue}">');
									...
									<div class="input-group-btn">
										...
										<ul class="dropdown-menu">
											...
											<li[#if page.searchProperty == "name"] class="active"[/#if] data-search-property="name">
												<a href="javascript:;">${message("Member.name")}</a>
											</li>
										</ul>
									</div>
							...
							<table class="table table-hover">
								<thead>
									<tr>
										...
										<th>
											<a href="javascript:;" data-order-property="isBaseMember">
												${message("Member.isBaseMember")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
									...
									<tbody>
										[#list page.content as member]
											<tr>
												...
												<td>
													[#if member.isBaseMember() == 1]
														是
													[/#if]
													[#if member.isBaseMember() == 0]
														否
													[/#if]
												</td>
												<td>
