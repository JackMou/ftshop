...
<head>
	...
	<title>${message("admin.role.list")}[#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.role.list")}</li>
			</ol>
			<form action="${base}/admin/role/list" method="get">
					...
					<div class="panel-heading">
						<div class="row">
							<div class="col-xs-12 col-sm-9">
								<div class="btn-group">
									<a class="btn btn-default" href="${base}/admin/role/add" data-redirect-url="${base}/admin/role/list">
							...
							<table class="table table-hover">
								<thead>
									<tr>
										...
										<th>
											<a href="javascript:;" data-order-property="isSystem">
												${message("Role.isSystem")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>${message("Role.description")}</th>
										<th>
									...
									<tbody>
										[#list page.content as role]
											<tr>
												...
												<td>
													[#if role.description??]
														<span title="${role.description}">${abbreviate(role.description, 30, "...")}</span>
													[/#if]
												</td>
												<td>
													<span title="${role.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${role.createdDate}</span>
												</td>
												<td>
													<a class="btn btn-default btn-xs btn-icon" href="${base}/admin/role/edit?id=${role.id}" title="${message("common.edit")}" data-toggle="tooltip" data-redirect-url>
