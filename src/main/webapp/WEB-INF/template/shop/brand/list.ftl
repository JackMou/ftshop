			...
			<div class="row">
				...
				<div class="col-xs-10">
					<ol class="breadcrumb">
						...
						<li class="active">${message("shop.brand.title")}</li>
					</ol>
					[#if page.content?has_content]
						<ul class="list clearfix">
							[#list page.content?sort_by("type") as brand]
								[#if brand.logo?has_content]
									<li>
										<a href="${base}${brand.path}" title="${brand.name}">
											<dl>
												...
												<dd class="text-overflow" title="${brand.name}">${brand.name}</dd>
											</dl>
										</a>
									</li>
								[#else]
									<li>
										<a class="text-overflow" href="${base}${brand.path}" title="${brand.name}">${brand.name}</a>
									</li>
								[/#if]
							[/#list]
						</ul>
					[#else]
						<div class="no-result">
							[#noautoesc]${message("shop.brand.noResult")}[/#noautoesc]
						</div>
					[/#if]
				</div>
			</div>
