...
<head>
	...
	<title>${message("business.store.register")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
...
<body class="business">
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				<li class="active">${message("business.store.register")}</li>
			</ol>
						...
						<div class="tab-content">
							<div id="info" class="tab-pane active">
								...
								<div class="form-group[#if !storeRanks??] hidden-element[/#if]">
									<label class="col-xs-3 col-sm-2 control-label item-required">${message("Store.storeRank")}:</label>
									<div class="col-xs-9 col-sm-4">
										<select id="storeRank" name="storeRankId" class="selectpicker form-control" data-size="5">
											[#list storeRanks as storeRank]
												[#if storeRank.isAllowRegister]
													<option value="${storeRank.id}" data-quantity="${(storeRank.quantity)!'${message("business.store.infiniteQuantity")}'}" data-service-fee="${storeRank.serviceFee}">${storeRank.name}</option>
												[/#if]
											[/#list]
										</select>
									</div>
								...
								<div class="form-group[#if !storeCategories??] hidden-element[/#if]">
									...
									<div class="col-xs-9 col-sm-4">
										<select id="storeCategory" name="storeCategoryId" class="selectpicker form-control" data-live-search="true" data-size="10">
											[#list storeCategories as storeCategory]
												<option value="${storeCategory.id}" data-bail="${storeCategory.bail}"[#if storeCategory.isDefault] selected[/#if]>${storeCategory.name}</option>
											[/#list]
										</select>
									</div>
								</div>
							...
							<div id="productCategorys" class="tab-pane">
								<div class="form-group">
									...
									<div class="col-xs-9 col-sm-4">
										<select name="productCategoryIds" class="selectpicker form-control" data-live-search="true" data-size="10" data-none-selected-text="${message("common.choose")}" multiple>
