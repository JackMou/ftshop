...
<head>
	...
	<title>${message("business.print.product")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
...
<body class="business print">
	...
	<main>
			...
			<tr>
				...
				<td colspan="2">
					...
					[#if order.promotionDiscount > 0]
						<p>${message("Order.promotionDiscount")}: ${currency(order.promotionDiscount, true, true)}</p>
					[/#if]
					[#if order.couponDiscount > 0]
						<p>${message("Order.couponDiscount")}: ${currency(order.couponDiscount, true, true)}</p>
					[/#if]
					[#if order.offsetAmount != 0]
						<p>${message("Order.offsetAmount")}: ${currency(order.offsetAmount, true, true)}</p>
					[/#if]
					<p>
		...
		<div class="text-right">Powered By futengwl.com</div>
	</main>
</body>
</html>