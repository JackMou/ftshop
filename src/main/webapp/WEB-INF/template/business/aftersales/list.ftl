...
<head>
	...
	<title>${message("business.aftersales.list")} [#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("business.aftersales.list")}</li>
			</ol>
			<form action="${base}/business/aftersales/list" method="get">
						...
						<div class="modal-content">
							...
							<div class="modal-body form-horizontal">
								<div class="form-group">
									<label class="col-xs-3 control-label">${message("business.aftersales.type")}:</label>
									<div class="col-xs-9 col-sm-7">
										<select name="type" class="selectpicker form-control" data-size="5">
											<option value="">${message("common.choose")}</option>
											[#list types as value]
												<option value="${value}"[#if value == type] selected[/#if]>${message("Aftersales.Type." + value)}</option>
											[/#list]
										</select>
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-3 control-label">${message("Aftersales.status")}:</label>
									<div class="col-xs-9 col-sm-7">
										<select name="status" class="selectpicker form-control" data-size="5">
											<option value="">${message("common.choose")}</option>
											[#list statuses as value]
												<option value="${value}"[#if value == status] selected[/#if]>${message("Aftersales.Status." + value)}</option>
											[/#list]
										</select>
									</div>
								</div>
							...
							<table class="table table-hover">
								<thead>
									<tr>
										...
										<th>${message("business.aftersales.type")}</th>
										<th>
									...
									<tbody>
										[#list page.content as aftersales]
											<tr>
												...
												<td>
													<a class="btn btn-default btn-xs btn-icon" href="${base}/business/aftersales/view?aftersalesId=${aftersales.id}" title="${message("common.view")}" data-toggle="tooltip">
														<i class="iconfont icon-search"></i>
													</a>
												</td>
											</tr>
