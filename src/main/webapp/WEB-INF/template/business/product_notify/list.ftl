...
<head>
	...
	<title>${message("business.productNotify.list")}[#if showPowered] - Powered By FTSHOP[/#if]</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				// 删除
				$delete.on("success.ftshop.delete", function() {
									...
									<tbody>
										[#list page.content as productNotify]
											<tr>
												...
												<td>
													<span title="${productNotify.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${productNotify.createdDate}</span>
												</td>
												<td>
													[#if productNotify.hasSent]
														<span title="${productNotify.lastModifiedDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${productNotify.lastModifiedDate}</span>
													[#else]
														-
													[/#if]
												</td>
