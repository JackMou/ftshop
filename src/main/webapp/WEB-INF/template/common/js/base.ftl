	...
	[#escape x as x?js_string]
		...
		+function($) {
			...
			CaptchaImage.prototype.init = function() {
				this.refresh();
				this.$captchaId = $('<input id="captchaId" name="captchaId" type="hidden" value="' + this.captchaId + '">').insertAfter(this.$element);
				this.$element.on("click.ftshop.captchaImage", $.proxy(this.refresh, this));
			};
			...
			CaptchaImage.prototype.refresh = function() {
				var refreshEvent = $.Event("refresh.ftshop.captchaImage");
				...
				this.$element.trigger("refreshed.ftshop.captchaImage");
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.captchaImage");
					var options = $.extend({}, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.captchaImage", (data = new CaptchaImage(this, options)));
					}
		...
		+function($) {
			...
			ScrollLoad.prototype.load = function() {
				...
				var loadEvent = $.Event("load.ftshop.scrollLoad");
				...
				$.ajax({
					...
					that.$element.trigger("loaded.ftshop.scrollLoad", arguments);
				}).fail(function() {
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.scrollLoad");
					var options = $.extend({}, ScrollLoad.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.scrollLoad", (data = new ScrollLoad(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
		...
		+function($) {
			...
			var AddCart = function(element, options) {
				...
				this.$element.on("click.ftshop.addCart", $.proxy(this.execute, this));
			};
			...
			AddCart.prototype.ajax = function() {
				...
				var recommenderId = typeof this.options.recommenderId === "function" ? this.options.recommenderId(this.$element) : this.options.recommenderId;
				return $.addCart(skuId, quantity,recommenderId);
			};
			...
			AddCart.prototype.execute = function() {
				var that = this;
				var beforeEvent = $.Event("before.ftshop.addCart");
				...
				this.ajax().done(function() {
					...
					that.fly(function() {
						that.$element.trigger("success.ftshop.addCart", ajaxArguments);
						that.$element.trigger("complete.ftshop.addCart", ajaxArguments);
					});
				}).fail(function() {
					that.$element.trigger("error.ftshop.addCart", arguments);
					that.$element.trigger("complete.ftshop.addCart", arguments);
				});
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.addCart");
					var options = $.extend({}, AddCart.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.addCart", (data = new AddCart(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.addCart.data-api", "[data-action='addCart']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.addCart") != null) {
					return;
				}
		...
		+function($) {
			...
			ModifyCart.prototype.changing = function(event, newValue, oldValue) {
				var that = this;
				var beforeEvent = $.Event("before.ftshop.modifyCart");
				...
				this.ajax(newValue).done(function() {
					that.$element.trigger("success.ftshop.modifyCart", arguments);
				}).fail(function() {
					that.$element.val(oldValue);
					that.$element.trigger("error.ftshop.modifyCart", arguments);
				}).always(function() {
					that.$element.trigger("complete.ftshop.modifyCart", arguments);
				});
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.modifyCart");
					var options = $.extend({}, ModifyCart.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.modifyCart", (data = new ModifyCart(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
		...
		+function($) {
			...
			var RemoveCart = function(element, options) {
				...
				this.$element.on("click.ftshop.removeCart", $.proxy(this.execute, this));
			};
			...
			RemoveCart.prototype.execute = function() {
				...
				var beforeEvent = $.Event("before.ftshop.removeCart");
				...
				if (confirm) {
					this.confirm(function(result) {
						...
						that.ajax().done(function() {
							that.$element.trigger("success.ftshop.removeCart", arguments);
						}).fail(function() {
							that.$element.trigger("error.ftshop.removeCart", arguments);
						}).always(function() {
							that.$element.trigger("complete.ftshop.removeCart", arguments);
						});
					});
				} else {
					this.ajax().done(function() {
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.removeCart");
					var options = $.extend({}, RemoveCart.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.removeCart", (data = new RemoveCart(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.removeCart.data-api", "[data-action='removeCart']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.removeCart") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var ClearCart = function(element, options) {
				...
				this.$element.on("click.ftshop.clearCart", $.proxy(this.execute, this));
			};
			...
			ClearCart.prototype.execute = function() {
				var that = this;
				var confirm = typeof this.options.confirm === "function" ? this.options.confirm(this.$element) : this.options.confirm;
				var beforeEvent = $.Event("before.ftshop.clearCart");
				...
				if (confirm) {
					this.confirm(function(result) {
						...
						that.ajax().done(function() {
							that.$element.trigger("success.ftshop.clearCart", arguments);
						}).fail(function() {
							that.$element.trigger("error.ftshop.clearCart", arguments);
						}).always(function() {
							that.$element.trigger("complete.ftshop.clearCart", arguments);
						});
					});
				} else {
					this.ajax().done(function() {
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.clearCart");
					var options = $.extend({}, ClearCart.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.clearCart", (data = new ClearCart(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.clearCart.data-api", "[data-action='clearCart']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.clearCart") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var Checkout = function(element, options) {
				...
				this.$element.on("click.ftshop.checkout", $.proxy(this.execute, this));
			};
			...
			Checkout.prototype.execute = function() {
				...
				var skuId = typeof this.options.skuId === "function" ? this.options.skuId(this.$element) : this.options.skuId;
				var quantity = typeof this.options.quantity === "function" ? this.options.quantity(this.$element) : this.options.quantity;
				var beforeEvent = $.Event("before.ftshop.checkout");
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.checkout");
					var options = $.extend({}, Checkout.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.checkout", (data = new Checkout(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.checkout.data-api", "[data-action='checkout']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.checkout") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var AddProductFavorite = function(element, options) {
				...
				this.$element.on("click.ftshop.addProductFavorite", $.proxy(this.execute, this));
			};
			...
			AddProductFavorite.prototype.execute = function() {
				var that = this;
				var beforeEvent = $.Event("before.ftshop.addProductFavorite");
				...
				this.ajax().done(function() {
					...
					that.shrinkOut(function() {
						that.$element.trigger("success.ftshop.addProductFavorite", ajaxArguments);
						that.$element.trigger("complete.ftshop.addProductFavorite", ajaxArguments);
					});
				}).fail(function() {
					that.$element.trigger("error.ftshop.addProductFavorite", arguments);
					that.$element.trigger("complete.ftshop.addProductFavorite", arguments);
				});
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.addProductFavorite");
					var options = $.extend({}, AddProductFavorite.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.addProductFavorite", (data = new AddProductFavorite(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.addProductFavorite.data-api", "[data-action='addProductFavorite']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.addProductFavorite") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var AddStoreFavorite = function(element, options) {
				...
				this.$element.on("click.ftshop.addStoreFavorite", $.proxy(this.execute, this));
			};
			...
			AddStoreFavorite.prototype.execute = function() {
				var that = this;
				var beforeEvent = $.Event("before.ftshop.addStoreFavorite");
				...
				this.ajax().done(function() {
					...
					that.shrinkOut(function() {
						that.$element.trigger("success.ftshop.addStoreFavorite", ajaxArguments);
						that.$element.trigger("complete.ftshop.addStoreFavorite", ajaxArguments);
					});
				}).fail(function() {
					that.$element.trigger("error.ftshop.addStoreFavorite", arguments);
					that.$element.trigger("complete.ftshop.addStoreFavorite", arguments);
				});
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.addStoreFavorite");
					var options = $.extend({}, AddStoreFavorite.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.addStoreFavorite", (data = new AddStoreFavorite(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.addStoreFavorite.data-api", "[data-action='addStoreFavorite']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.addStoreFavorite") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var Delete = function(element, options) {
				...
				this.$element.on("click.ftshop.delete", $.proxy(this.execute, this));
			};
			...
			Delete.prototype.execute = function() {
				var that = this;
				var confirm = typeof this.options.confirm === "function" ? this.options.confirm(this.$element) : this.options.confirm;
				var beforeEvent = $.Event("before.ftshop.delete");
				...
				if (confirm) {
					this.confirm(function(result) {
						...
						that.ajax().done(function() {
							that.$element.trigger("success.ftshop.delete", arguments);
						}).fail(function() {
							that.$element.trigger("error.ftshop.delete", arguments);
						}).always(function() {
							that.$element.trigger("complete.ftshop.delete", arguments);
						});
					});
				} else {
					this.ajax().done(function() {
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.delete");
					var options = $.extend({}, Delete.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.delete", (data = new Delete(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.delete.data-api", "[data-action='delete']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.delete") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var Refresh = function(element, options) {
				...
				this.$element.on("click.ftshop.refresh", $.proxy(this.execute, this));
			};
			...
			Refresh.prototype.execute = function() {
				...
				var beforeEvent = $.Event("before.ftshop.refresh");
				...
				this.$element.trigger("complete.ftshop.refresh");
			};
			
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.refresh");
					var options = $.extend({}, Refresh.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.refresh", (data = new Refresh(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.refresh.data-api", "[data-action='refresh']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.refresh") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var CheckAll = function(element, options) {
				...
				this.$element.on("change.ftshop.checkAll", $.proxy(this.toggle, this));
			};
			...
			CheckAll.prototype.check = function() {
				...
				var checkEvent = $.Event("check.ftshop.checkAll");
				...
				this.$element.trigger("checked.ftshop.checkAll");
			};
			
			CheckAll.prototype.uncheck = function() {
				var target = typeof this.options.target === "function" ? this.options.target(this.$element) : this.options.target;
				var $target = target instanceof jQuery ? target : $(target);
				var uncheckEvent = $.Event("uncheck.ftshop.checkAll");
				...
				this.$element.trigger("unchecked.ftshop.checkAll");
			};
			
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.checkAll");
					var options = $.extend({}, CheckAll.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.checkAll", (data = new CheckAll(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("change.ftshop.checkAll.data-api", "[data-toggle='checkAll']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.checkAll") != null) {
					return;
				}
		...
		+function($) {
			...
			var Back = function(element, options) {
				...
				this.$element.on("click.ftshop.back", $.proxy(this.execute, this));
			};
			
			Back.prototype.execute = function() {
				var beforeEvent = $.Event("before.ftshop.back");
				...
				this.$element.trigger("complete.ftshop.back");
			};
			
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("ftshop.back");
					var options = $.extend({}, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("ftshop.back", (data = new Back(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.ftshop.back.data-api", "[data-action='back']", function() {
				var $this = $(this);
				
				if ($this.data("ftshop.back") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		(function($) {
			...
			// 加入购物车
			$.addCart = function(skuId, quantity, recommenderId) {
			...
			$.generateSpreadUrl = function() {
				...
				return currentUser != null ? url.replace(/(#.*)?$/, "#SPREAD_" + $.base64.encode(currentUser.username)) : null;
			}
			...
			$.fn.ajaxSubmit.defaults = {
				...
				beforeSubmit: function(arr, $form, options) {
					var beforeEvent = $.Event("before.ftshop.ajaxSubmit", arguments);
				...
				success: function(data, textStatus, xhr, $form) {
					...
					$form.trigger("succ" + "ess.ftsho" + "p" + ".ajaxSubmit", arguments);
				...
				error: function(xhr, textStatus, error, $form) {
					$form.find(":submit").prop("disabled", false).removeClass("btn-before-submit");
					$form.trigger("error.ftshop.ajaxSubmit", arguments);
				},
				complete: function(xhr, textStatus, $form) {
					$form.trigger("complete.ftshop.ajaxSubmit", arguments);
				}
			};
		...
		$().ready(function() {
			...
			// 用户注册/登录
			$document.on("registered.ftshop.user loggedIn.ftshop.user", function(event, currentUser) {
				$.setCurrentUser(currentUser);
				$.removeCurrentCartQuantity();
			});
			
			// 用户注销
			$document.on("loggedOut.ftshop.user", function(event, currentUser) {
				$.removeCurrentUser();
				$.removeCurrentCartQuantity();
			});
			...
			if ($.getCurrentCartQuantity() == null) {
				$.getCurrentCart().done(function(data) {
					...
					var beforeEvent = $.Event("before.ftshop.setCurrentCartQuantity");
					...
					$document.trigger("complete.ftshop.setCurrentCartQuantity", quantity);
				});
			}
			
			// 加入/修改/移除/清空购物车
			$document.on("success.ftshop.addCart success.ftshop.modifyCart success.ftshop.removeCart success.ftshop.clearCart", function(event, data) {
				var quantity = data.quantity != null ? data.quantity : 0;
				var beforeEvent = $.Event("before.ftshop.setCurrentCartQuantity");
